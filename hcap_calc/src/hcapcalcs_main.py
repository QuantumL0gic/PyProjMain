...
######################################
# Column names
def diff_calculator(dat_score, dat_course, dat_direction):
    ...
    dat_hcap['Date'] = pd.to_datetime(dat_hcap.Date)
...
def calc_hidx_cidx(dat_hcap, calc_noninclusive):
    ...
    if calc_noninclusive == 'Y':    
        nones_index = dat_hcap.loc[dat_hcap['Inclusion'] == 0].index.values
